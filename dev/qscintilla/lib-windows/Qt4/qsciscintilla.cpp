        ...
        bool checkThresh, bool choose_single)
{
    ...
    if (acs == AcsAll || acs == AcsDocument)
    {
		...
		/* Get the keywords of the lexer */
		QString lexerKeywords;
		int indexSet;
		for(indexSet = 0; indexSet <= KEYWORDSET_MAX; indexSet++)
		{
			if(lex->keywords(indexSet)) 
			{
				lexerKeywords.append(lex->keywords(indexSet));
				lexerKeywords.append(" ");
			}
		}
		
		/* Add keywords to the list */
		int indexSpaceChar = 0;
		int indexPredChar = 0;
		while(lexerKeywords.indexOf(last_data, indexPredChar, Qt::CaseInsensitive) != -1)
		{	
			indexPredChar = lexerKeywords.indexOf(last_data, indexPredChar, Qt::CaseInsensitive);
			while(lexerKeywords[indexPredChar+indexSpaceChar] != ' ')
				indexSpaceChar++;		
			QString res1 =  lexerKeywords.mid(indexPredChar, indexSpaceChar);
			if((indexPredChar == 0 || lexerKeywords[indexPredChar-1] == ' ') && !wlist.contains(res1))
				wlist.append(res1);
			indexPredChar += indexSpaceChar;
			indexSpaceChar = 0;
		}
