...
LangString UserAdmin_Label ${LANG_ENGLISH} "You are running this installer as User. Write access to certain folders (e.g. C:\Program Files\) is restricted. If the installation does not complete, select another installtion folder or restart the installer with Administrator rights. Do you want to continue?"
LangString UserAdmin_Label ${LANG_FRENCH}  "Copie les fichiers nécessaires dans le dossier de destination."
LangString UserAdmin_Label ${LANG_GERMAN}  "Kopiere die benötigten Dateien in den Zielordner."
LangString UserAdmin_Label ${LANG_ITALIAN} "Copia i file necessari nella cartella di destinazione."
LangString UserAdmin_Label ${LANG_SPANISH} "Copia los archivos necesarios en la carpeta de destino."
...
Function portableModePageCreate
    ...
    ${If} $PortableMode = 0
        ## Label User Warning
        ${NSD_CreateLabel}       10u 10u 240u 16u "Warning be here"
        Pop $0
    ${EndIf}
    # Group Normal
    ${NSD_CreateGroupBox}    10u  40u 280u 46u ""
    Pop $0
    ## Radio Normal
    ${NSD_CreateRadioButton} 20u  50u -30u  8u "$(InstMode_Registered)"
    Pop $1
    SendMessage $1 ${WM_SETFONT} $boldFont 0
    ## Label Normal
    ${NSD_CreateLabel}       31u  66u 240u 16u "$(InstMode_Registered_Label)"
    Pop $0
    # Group Portable
    ${NSD_CreateGroupBox}    10u  90u 280u 46u ""
    Pop $0
    ## Radio Portable
    ${NSD_CreateRadioButton} 20u  100u -30u  8u "$(InstMode_Portable)"
    Pop $2
    SendMessage $2 ${WM_SETFONT} $boldFont 0
    ## Label Portable
    ${NSD_CreateLabel}       31u 116u 240u 16u "$(InstMode_Portable_Label)"
    Pop $0
    # Store selection
    ${If} $PortableMode = 0
...
;    UserInfo::GetAccountType
    ...
    ${If} ${Errors}
        MessageBox MB_YESNO "$(UserAdmin_Label)" IDYES continue IDNO quit
        quit:
            Quit
        continue:
        Abort
    ...
    ; ${If} $8 == "admin"
        ; !ifdef INSTALLER_DEBUG
            ; MessageBox MB_ICONINFORMATION "All access"
        ; !endif
    ; ${Else}
        ; MessageBox MB_YESNO "$(UserAdmin_Label)" IDYES continue IDNO quit
        ; quit:
